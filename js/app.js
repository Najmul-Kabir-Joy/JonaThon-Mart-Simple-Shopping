...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    //html code for displaying a single item
    ...
    <img class="product-image" src=${image}></img>
      ...
      <div id="rating">
        <h6><i class="fas fa-star"></i>${product.rating.rate}</h6>
        <h6><i class="fas fa-user-check"></i>${product.rating.count}</h6>
      </div>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button id="details-btn" class="btn btn-danger" onclick="openModal('${title}', '${image}',
      '${product.rating.rate}','${product.rating.count}','${product.price}','${product.id}')"  data-toggle="modal" data-target="#myModal2">Details</button></div>
      `;
    document.getElementById("all-products").appendChild(div);
  }
};
//adding products to cart and updating them
let count = 0;
...
//code for modal. The modal will open when you click on the details button
const openModal = (title, image, rate, count, price, id) => {
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div>
          <img class="product-image" src=${image}  style="display: block; margin: auto;"></img>
        </div>
        <h3  style="text-align: center">${title}</h3>
        <div id="rating">
          <h6><i class="fas fa-star"></i>${rate}</h6>
          <h6><i class="fas fa-user-check"></i>${count}</h6>
        </div>
        <h2 style="text-align: center">Price: $ ${price}</h2>
      </div>
      <div class="modal-footer">
      <button onclick="addToCart(${id},${price})" id="addToCart-btn" class="buy-now btn btn-success" data-dismiss="modal">add to cart</button>
